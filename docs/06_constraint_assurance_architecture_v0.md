...
人工审查只处理机器不能可靠判定或风险过高的事项：需求归属、模型代表性、额定参数、证据适用范围、第三方资料许可、是否可以进入正式基线或学习流程。审查结果必须结构化记录，不能只留在聊天或口头会议中。
...
- 状态机 Guard、人工审批、比较准入、学习准入和可视化追溯。
...
4. 增加结构化人工审查和审批文件，并把发布/学习入口设为 fail-closed。
