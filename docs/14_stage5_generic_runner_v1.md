...
- Every run records `CREATED -> RUNNING -> RUN_OK`, `RUN_FAILED`, or
  `INCOMPLETE` in `state_transitions.json`; each transition has a reason,
...
- `sample_offset_s` is implemented as a pre-sample hold using the previous
  action. `plant_step_s` segments each plant advance. Action target timestamps
  are validated and applied within the current control window.
