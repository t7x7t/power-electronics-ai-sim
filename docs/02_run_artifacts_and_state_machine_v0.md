...
- `LEARNING_ELIGIBLE` 表示在启用可选学习/适配扩展且通过额外准入审查时，允许将证据交给外部学习器；它不是核心 Runner 的必经状态；
- 任何安全失败、非有限值、未来信息泄漏或合同不匹配都禁止进入学习。
- 未接入学习/适配扩展时，运行在 `QUALIFIED` 或 `COMPARABLE` 即可完成核心基础设施流程；项目不内置训练器、在线学习循环或自动模型/控制器更新。
